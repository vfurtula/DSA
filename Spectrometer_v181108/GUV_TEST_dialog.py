...
class GUV_TEST_dialog(QDialog):
	...
	def __init__(self, parent):
		...
		try:
			...
			self.log_guv_str=self.config.get('DEFAULT','log_guv').strip().split(',')[0]
			self.log_guv_check=self.bool_(self.config.get('DEFAULT','log_guv').strip().split(',')[1])
	...
	def set_run(self):
		...
		if self.cb_logdata.isChecked():
			...
			self.config.set('DEFAULT', 'log_guv', ','.join([self.log_guv_str, str(True)]) )
		else:
			self.config.set('DEFAULT', 'log_guv', ','.join([self.log_guv_str, str(False)]) )
