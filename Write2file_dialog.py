...
class Write2file_dialog(QDialog):
	...
	def __init__(self, parent):
		...
		try:
			...
			self.w2hdf5File_str=self.config.get(self.last_used_scan,'write2hdf5').strip().split(',')[0]
			self.w2hdf5_check=self.bool_(self.config.get(self.last_used_scan,'write2hdf5').strip().split(',')[1])
	...
	def initUI(self):
		...
		w2hdf5_lbl = QLabel("Write to a hdf5 file (.hdf5):",self)
		w2hdf5_lbl.setStyleSheet("color: blue")
		self.w2hdf5FileEdit = QLineEdit(self.w2hdf5File_str,self)
		self.w2hdf5FileEdit.textChanged.connect(self.on_text_changed)
		self.w2hdf5FileEdit.setEnabled(self.w2hdf5_check)
		self.w2hdf5FileEdit.setFixedWidth(250)
		self.cb_w2hdf5 = QCheckBox('',self)
		self.cb_w2hdf5.toggle()
		self.cb_w2hdf5.setChecked(self.w2hdf5_check)
		...
		g0_0.addWidget(w2hdf5_lbl,9,0)
		g0_0.addWidget(self.cb_w2hdf5,9,1)
		g0_0.addWidget(self.w2hdf5FileEdit,10,0)
    ...
    ##############################################
		...
		self.cb_w2hdf5.stateChanged.connect(self.w2hdf5_stch)
	...
	def w2hdf5_stch(self, state):
		
		self.on_text_changed()
		if state in [Qt.Checked,True]:
			self.w2hdf5FileEdit.setEnabled(True)
		else:
			self.w2hdf5FileEdit.setEnabled(False)
	...
	def save_(self):
		...
		self.config.set(self.last_used_scan, 'write2hdf5', ','.join([str(self.w2hdf5FileEdit.text()), str(self.cb_w2hdf5.isChecked()) ]) )
