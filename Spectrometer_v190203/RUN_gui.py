...
import traceback, sys, os, sqlite3, itertools, collections, configparser
...
class Data_Worker(QRunnable):
	...
	def read_signals(self,*tupl):
		...
		while (time.time()-time_start_)<self.dwell and not self.abort_flag:
			...
			if self.inst_list.get('Agilent34972A'):
				V_a34972a=format(self.inst_list.get('Agilent34972A').return_voltage(self.pass_wl),'.6e')
		...
		if V_a34972a is not None:
			val=[]
			for i in range(self.avgpts):
				val.extend([self.inst_list.get('Agilent34972A').return_voltage(self.pass_wl)])
				if self.abort_flag:
					self.close_shutter()
					return
...
class Run_TEST(QMainWindow):
	...
	def set_run(self):
		...
		self.md = Indicator_invs_dialog.Indicator_invs_dialog(self, "...preparing scan...", "green", "indicators/ajax-loader-green-ball2.gif")
	...
	def update_end_k2001a(self,pyqt_object):
		...
		if self.write2db_check:
			...
			self.db.execute(''.join(["INSERT INTO keithley VALUES ('",str(ms257_in),"','", str(ms257_out),"',", str(abs(volt)), ",'", self.posnow,"','", self.shutnow, "',", time, ",'", dateandtime,"','", my_legend ,"')"]))
	...
	def update_end_a34972a(self,pyqt_object):
		...
		if self.write2db_check:
			# Save to a sqlite3 file
			'''CREATE TABLE spectra (wavelength real, voltage real, position text, shutter real, timetrace real, absolutetime text)'''
			self.db.execute(''.join(["INSERT INTO agilent VALUES ('",str(ms257_in),"','", str(ms257_out),"',", str(abs(volt)), ",'", self.posnow,"','", self.shutnow, "',", time, ",'", dateandtime,"','", my_legend,"')"]))
			# Save (commit) the changes
			self.conn.commit()
	...
	def update_end_guv(self,pyqt_object):
		...
		if self.write2db_check:
			# Save to a sqlite3 file
			'''CREATE TABLE spectra (wavelength real, voltage real, position text, shutter real, timetrace real, absolutetime text)'''
			self.db.execute(''.join(["INSERT INTO guv VALUES ('", self.guvtype_str ,"','",str(ms257_in),"','",str(ms257_out),"','", str(list(abs(i) for i in volt)),"','", str(list(int(i) for i in self.guv_channels)),"','",my_legend,"','", self.posnow,"','", self.shutnow, "',", time, ",'", dateandtime, "')"]))
			# Save (commit) the changes
			self.conn.commit()
