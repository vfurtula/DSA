...
class MS257_dialog(QDialog):
	...
	def __init__(self, parent, config, inst_list):
		...
		self.grating = self.config.get('DEFAULT','grating')
	...
	def onActivated0(self, text):
		if not hasattr(self, 'MS257'):
			...
			try:
				...
				QMessageBox.warning(self, 'Message',"Something went wrong with the MS257 serial port!")
				self.combo0.setCurrentIndex(self.mylist0.index(self.grating))
				return
			self.MS257.set_timeout(60)
	...
	def onActivated3(self, text):
		if not hasattr(self, 'MS257'):
			...
			try:
				...
				QMessageBox.warning(self, 'Message',"Somethiong went wrong with the MS257 serial port!")
				self.combo3.setCurrentIndex(self.mylist3.index(self.shutter_str))
				return
			self.MS257.set_timeout(60)
