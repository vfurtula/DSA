				...
				if self.pos=="A and B":
					...
					if self.shutter=="on and off":
						...
						if self.inst_list.get('MS257_in'):
							...
							self.signals.update_shutter.emit(self.shutter)
						else:
							self.signals.update_shutter.emit("?")
						time_start_=time.time()
						while (time.time()-time_start_)<self.shutdelay and not self.abort_flag:
...
class Run_TEST(QMainWindow):
	...
	def initUI(self):
		...
		self.c_k2001a_end_Anone = self.p0.plot(pen=pg.mkPen('m', style=QtCore.Qt.DotLine))
		...
		self.c_k2001a_end_Bnone = self.p0.plot(pen=pg.mkPen('m', style=QtCore.Qt.DotLine))
		...
		self.c_k2001a_end_Nnone = self.p0.plot(pen=pg.mkPen('m', style=QtCore.Qt.DotLine))
		...
		self.c_a34972a_end_Anone = self.p0.plot(pen=pg.mkPen('c', style=QtCore.Qt.DotLine))
		...
		self.c_a34972a_end_Bnone = self.p0.plot(pen=pg.mkPen('c', style=QtCore.Qt.DotLine))
		...
		self.c_a34972a_end_Nnone = self.p0.plot(pen=pg.mkPen('c', style=QtCore.Qt.DotLine))
		#self.my_legend.hide()
		...
		self.c_k2001a_all_Anone = self.p1.plot(pen=pg.mkPen('m', style=QtCore.Qt.DotLine))
		...
		self.c_k2001a_all_Bnone = self.p1.plot(pen=pg.mkPen('m', style=QtCore.Qt.DotLine))
		...
		self.c_k2001a_all_Nnone = self.p1.plot(pen=pg.mkPen('m', style=QtCore.Qt.DotLine))
		...
		self.c_a34972a_all_Anone = self.p1.plot(pen=pg.mkPen('c', style=QtCore.Qt.DotLine))
		...
		self.c_a34972a_all_Bnone = self.p1.plot(pen=pg.mkPen('c', style=QtCore.Qt.DotLine))
		...
		self.c_a34972a_all_Nnone = self.p1.plot(pen=pg.mkPen('c', style=QtCore.Qt.DotLine))
		...
		hbox2.addWidget(self.tableWidget,2)
	...
	def createTable(self):
		...
		tableWidget.setHorizontalHeaderLabels(["Detector","Wavel.[nm]","U[V]","Pos","Shut","T[s]"])
	...
	def instrumentsDialog(self):
		...
		if not self.inst_list.get('Oriel'):
			...
			self.aorb=self.pos_str
		
		if not self.inst_list.get('MS257_in'):
			self.onoff="?"
		else:
			self.onoff=self.shutter_str
	...
	def prepare_file(self):
		...
		if self.inst_list.get('K2001A') or self.inst_list.get('Agilent34972A'):
			...
			if self.write2db_check:
				...
				self.db.execute('''CREATE TABLE spectra (detector text, ms257_in text, ms257_out text, voltage real, position text, shutter text, timetrace real, absolutetime text)''')
	...
	def set_run(self):
		...
		if not bool(self.inst_list):
			QMessageBox.warning(self, 'Message',"No instruments connected. Choose at least 1 instrument.")
			return
		...
		if "A" in self.aorb:
			...
			if "?" in self.onoff:
				if self.inst_list.get('K2001A'):
					self.my_legend.addItem(self.c_k2001a_end_Anone, name='Keithley,A-?')
				if self.inst_list.get('Agilent34972A'):
					self.my_legend.addItem(self.c_a34972a_end_Anone, name='Agilent,A-?')
		...
		if "B" in self.aorb:
			...
			if "?" in self.onoff:
				if self.inst_list.get('K2001A'):
					self.my_legend.addItem(self.c_k2001a_end_Bnone, name='Keithley,B-?')
				if self.inst_list.get('Agilent34972A'):
					self.my_legend.addItem(self.c_a34972a_end_Bnone, name='Agilent,B-?')
		...
		if "?" in self.aorb:
			...
			if "?" in self.onoff:
				if self.inst_list.get('K2001A'):
					self.my_legend.addItem(self.c_k2001a_end_Nnone, name='Keithley,?-?')
				if self.inst_list.get('Agilent34972A'):
					self.my_legend.addItem(self.c_a34972a_end_Nnone, name='Agilent,?-?')
	...
	def update_end_k2001a(self,pyqt_object):
		...
		if self.is_number(wl):
			...
			if "A" in self.aorb:
				...
				if "?" in self.onoff:
					self.wl_k2001a_end_Anone.extend([ float(wl) ])
					self.volt_k2001a_end_Anone.extend([ float(volt) ])
					self.c_k2001a_end_Anone.setData(self.wl_k2001a_end_Anone, self.volt_k2001a_end_Anone)
			...
			if "B" in self.aorb:
				...
				if "?" in self.onoff:
					self.wl_k2001a_end_Bnone.extend([ float(wl) ])
					self.volt_k2001a_end_Bnone.extend([ float(volt) ])
					self.c_k2001a_end_Bnone.setData(self.wl_k2001a_end_Bnone, self.volt_k2001a_end_Bnone)
			...
			if "?" in self.aorb:
				...
				if "?" in self.onoff:
					self.wl_k2001a_end_Nnone.extend([ float(wl) ])
					self.volt_k2001a_end_Nnone.extend([ float(volt) ])
					self.c_k2001a_end_Nnone.setData(self.wl_k2001a_end_Nnone, self.volt_k2001a_end_Nnone)
		...
		self.pos_.extend([self.aorb])
		self.shutter_.extend([self.onoff])
		...
		if self.write2db_check:
			...
			self.db.execute(''.join(["INSERT INTO spectra VALUES ('", "Keithley" ,"','", ms257_in,"','", ms257_out,"',", volt, ",'", self.aorb,"','", self.onoff, "',", time, ",'", dateandtime, "')"]))
		...
		if self.write2txt_check:
			...
			with open(self.textfile, 'a') as thefile:
				...
				thefile.write("\t%s" %self.aorb)
				thefile.write("\t%s" %self.onoff)
		...
		self.tableWidget.setItem(self.tal, 3, QTableWidgetItem(self.aorb) )
		self.tableWidget.setItem(self.tal, 4, QTableWidgetItem(self.onoff) )
	...
	def update_end_a34972a(self,pyqt_object):
		...
		if self.is_number(wl):
			...
			if "A" in self.aorb:
				...
				if "?" in self.onoff:
					self.wl_a34972a_end_Anone.extend([ float(wl) ])
					self.volt_a34972a_end_Anone.extend([ float(volt) ])
					self.c_a34972a_end_Anone.setData(self.wl_a34972a_end_Anone, self.volt_a34972a_end_Anone)
			...
			if "B" in self.aorb:
				...
				if "?" in self.onoff:
					self.wl_a34972a_end_Bnone.extend([ float(wl) ])
					self.volt_a34972a_end_Bnone.extend([ float(volt) ])
					self.c_a34972a_end_Bnone.setData(self.wl_a34972a_end_Bnone, self.volt_a34972a_end_Bnone)
			...
			if "?" in self.aorb:
				...
				if "?" in self.onoff:
					self.wl_a34972a_end_Nnone.extend([ float(wl) ])
					self.volt_a34972a_end_Nnone.extend([ float(volt) ])
					self.c_a34972a_end_Nnone.setData(self.wl_a34972a_end_Nnone, self.volt_a34972a_end_Nnone)
		...
		if self.write2db_check:
			# Save to a database file
			'''CREATE TABLE spectra (wavelength real, voltage real, position text, shutter real, timetrace real, absolutetime text)'''
			self.db.execute(''.join(["INSERT INTO spectra VALUES ('", "Agilent" ,"','",ms257_in,"','", ms257_out,"',", volt, ",'", self.aorb,"','", self.onoff, "',", time, ",'", dateandtime, "')"]))
			# Save (commit) the changes
			self.conn.commit()
	...
	def update_all_k2001a(self,pyqt_object):
		...
		if "A" in self.aorb:
			...
			if "?" in self.onoff:
				self.time_k2001a_all_Anone.extend([ float(time) ])
				self.volt_k2001a_all_Anone.extend([ float(volt) ])
				self.c_k2001a_all_Anone.setData(self.time_k2001a_all_Anone, self.volt_k2001a_all_Anone)
		...
		if "B" in self.aorb:
			...
			if "?" in self.onoff:
				self.time_k2001a_all_Bnone.extend([ float(time) ])
				self.volt_k2001a_all_Bnone.extend([ float(volt) ])
				self.c_k2001a_all_Bnone.setData(self.time_k2001a_all_Bnone, self.volt_k2001a_all_Bnone)
		...
		if "?" in self.aorb:
			...
			if "?" in self.onoff:
				self.time_k2001a_all_Nnone.extend([ float(time) ])
				self.volt_k2001a_all_Nnone.extend([ float(volt) ])
				self.c_k2001a_all_Nnone.setData(self.time_k2001a_all_Nnone, self.volt_k2001a_all_Nnone)
	...
	def update_all_a34972a(self,pyqt_object):
		...
		if "A" in self.aorb:
			...
			if "?" in self.onoff:
				self.time_a34972a_all_Anone.extend([ float(time) ])
				self.volt_a34972a_all_Anone.extend([ float(volt) ])
				self.c_a34972a_all_Anone.setData(self.time_a34972a_all_Anone, self.volt_a34972a_all_Anone)
		...
		if "B" in self.aorb:
			...
			if "?" in self.onoff:
				self.time_a34972a_all_Bnone.extend([ float(time) ])
				self.volt_a34972a_all_Bnone.extend([ float(volt) ])
				self.c_a34972a_all_Bnone.setData(self.time_a34972a_all_Bnone, self.volt_a34972a_all_Bnone)
		...
		if "?" in self.aorb:
			...
			if "?" in self.onoff:
				self.time_a34972a_all_Nnone.extend([ float(time) ])
				self.volt_a34972a_all_Nnone.extend([ float(volt) ])
				self.c_a34972a_all_Nnone.setData(self.time_a34972a_all_Nnone, self.volt_a34972a_all_Nnone)
	...
	def update_wl_time(self,pyqt_object):
		...
		if not self.inst_list.get('K2001A') and not self.inst_list.get('Agilent34972A'):
			self.p2.setGeometry(self.p1.vb.sceneBoundingRect())
	...
	def update_shutter(self,onoff):
		...
		self.onoff = onoff
		self.shind_lbl.setText(''.join(["Shutter ", self.onoff.upper()]))
		if self.onoff=="on":
	...
	def update_oriel(self,aorb):
		...
		self.aorb = aorb
		self.posind_lbl.setText(''.join(["Position ", self.aorb]))
		if self.aorb=="A":
