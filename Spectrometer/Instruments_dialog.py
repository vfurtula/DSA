...
class Instruments_dialog(QDialog):
	...
	def initUI(self):
		...
		self.guvipEdit.setFixedWidth(225)
    ...
    ##############################################
		...
		if self.inst_list.get("ALL_DISCON")==True:
	...
	def set_connect(self):
		...
		if self.cb_ims257.isChecked():
			...
			try:
				...
				self.ims257_status.setText("Status: Problems with the port!")
				self.ims257_status.setStyleSheet("color: red")
		...
		if self.cb_oms257.isChecked():
			...
			try:
				...
				self.oms257_status.setText("Status: Problems with the port!")
				self.oms257_status.setStyleSheet("color: red")
		...
		if self.cb_a34972a.isChecked():
			try:
				...
				self.a34972a_status.setText("Agilent34972A status: CONNECTED")
				self.a34972a_status.setStyleSheet("color: green")
				self.inst_list.update({'Agilent34972A':self.Agilent34972A})
			except Exception as e:
				QMessageBox.warning(self, 'Message',"Agilent34972A could not return valid echo signal.\nCheck the port name and check the connection.")
				self.a34972a_status.setText("Status: Port exists but no communication to the device!")
				self.a34972a_status.setStyleSheet("color: red")
		else:
		...
		if self.cb_guv.isChecked():
			try:
				self.GUV = GUV.GUV([self.guvip_str,self.guvport_str])
				
				print(self.GUV.return_id())
				self.guv_status.setText("GUV status: CONNECTED")
				self.guv_status.setStyleSheet("color: green")
				self.inst_list.update({'GUV':self.GUV})
			except Exception as e:
				QMessageBox.warning(self, 'Message',"GUV could not return valid echo signal.\nCheck the port name and check the connection.")
				self.guv_status.setText("Status: Port exists but no communication to the device!")
				self.guv_status.setStyleSheet("color: red")
		else:
			if self.inst_list.get("GUV") or hasattr(self,"GUV"):
