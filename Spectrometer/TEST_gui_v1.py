...
class TEST_Worker(QRunnable):
	...
	def run(self):
		...
		# Retrieve args/kwargs here; and fire processing using them
		try:
			if self.inst_list.get('MS257_in') and self.inst_list.get('MS257_out') and self.inst_list.get('K2001A') and self.inst_list.get('Agilent34972A') and self.inst_list.get('Oriel'):
				self.ms257_in = self.inst_list.get('MS257_in')
				self.ms257_out = self.inst_list.get('MS257_out')
				self.k2001a = self.inst_list.get('K2001A')
				self.a34972a = self.inst_list.get('Agilent34972A')
				self.oriel = self.inst_list.get('Oriel')
				self.run_sc0()
				
			elif self.inst_list.get('MS257_in') and self.inst_list.get('MS257_out') and self.inst_list.get('K2001A') and self.inst_list.get('Oriel'):
				self.ms257_in = self.inst_list.get('MS257_in')
				self.ms257_out = self.inst_list.get('MS257_out')
				self.k2001a = self.inst_list.get('K2001A')
				self.oriel = self.inst_list.get('Oriel')
			...
			elif self.inst_list.get('MS257_in') and self.inst_list.get('MS257_out') and self.inst_list.get('Oriel'):
				self.ms257_in = self.inst_list.get('MS257_in')
				self.ms257_out = self.inst_list.get('MS257_out')
				self.oriel = self.inst_list.get('Oriel')
				self.run_sc2()
			...
			elif self.inst_list.get('MS257_in') and self.inst_list.get('MS257_out') and self.inst_list.get('Agilent34972A') and self.inst_list.get('Oriel'):
				self.ms257_in = self.inst_list.get('MS257_in')
				self.ms257_out = self.inst_list.get('MS257_out')
				self.a34972a = self.inst_list.get('Agilent34972A')
				self.oriel = self.inst_list.get('Oriel')
			...
			elif self.inst_list.get('MS257_in') and self.inst_list.get('MS257_out') and self.inst_list.get('K2001A') and self.inst_list.get('GUV') and self.inst_list.get('Oriel'):
				self.ms257_in = self.inst_list.get('MS257_in')
				self.ms257_out = self.inst_list.get('MS257_out')
				self.k2001a = self.inst_list.get('K2001A')
				self.guv = self.inst_list.get('GUV')
				self.oriel = self.inst_list.get('Oriel')
				self.run_sc5()
			
			elif self.inst_list.get('MS257_in') and self.inst_list.get('MS257_out') and self.inst_list.get('GUV') and self.inst_list.get('Oriel'):
				self.ms257_in = self.inst_list.get('MS257_in')
				self.ms257_out = self.inst_list.get('MS257_out')
				self.guv = self.inst_list.get('GUV')
				self.oriel = self.inst_list.get('Oriel')
	...
	def run_sc0(self):
		...
		# set Agilent34972A supply
		self.k2001a.set_dc_voltage()
		self.a34972a.set_dc_voltage()
				...
				if self.pos=="A and B":
					...
					if self.shutter=="on and off":
						...
						while (time.time()-time_start_)<dwell and not self.abort_flag:
							...
							#time.sleep(0.1)
							X_val1=format(self.k2001a.return_voltage(),'.6e')
							X_val2=format(self.a34972a.return_voltage(),'.6e')
							self.signals.update2.emit([step_wl,[X_val1,X_val2],time_elap])
				...
				if self.pos=="A and B":
					...
					if self.shutter=="on and off":
						...
						while (time.time()-time_start_)<dwell and not self.abort_flag:
							...
							#X_val=format(random.random(), '07.4f')
							#time.sleep(0.1)
							X_val=format(self.k2001a.return_voltage(),'.6e')
...
class Run_TEST(QMainWindow):
	...
	def initUI(self):
		...
		self.p0.setLabel('left', "Keithley", units='V', color='green')
		self.p0.hideAxis('bottom')
		
		self.win2.nextRow()
		
		self.p3 = self.win2.addPlot()
		self.my_legend2 = self.p3.addLegend()
		self.curve5_Aon = self.p3.plot(pen='y', symbol='s', symbolPen='y', symbolBrush='b', symbolSize=6)
		self.curve5_Aoff = self.p3.plot(pen=pg.mkPen('y', style=QtCore.Qt.DashLine))
		self.curve5_Bon = self.p3.plot(pen='m', symbol='s', symbolPen='m', symbolBrush='b', symbolSize=6)
		self.curve5_Boff = self.p3.plot(pen=pg.mkPen('m', style=QtCore.Qt.DashLine))
		self.curve5_Non = self.p3.plot(pen='c', symbol='s', symbolPen='c', symbolBrush='b', symbolSize=6)
		self.curve5_Noff = self.p3.plot(pen=pg.mkPen('c', style=QtCore.Qt.DashLine))
		#self.my_legend.hide()
		
		self.p3.enableAutoRange()
		self.p3.setLabel('left', "Agilent/GUV", units='V', color='blue')
		self.p3.setLabel('bottom', "Wavelength", units='m', color='white')
		...
		self.curve5=self.p1.plot(pen=pg.mkPen('r', style=QtCore.Qt.DashLine))
		self.curve6=self.p1.plot(pen=pg.mkPen('w', style=QtCore.Qt.DashLine))
	...
	def instrumentsDialog(self):
		...
		if self.inst_list.get('MS257_in') and self.inst_list.get('MS257_out') and self.inst_list.get('K2001A') and self.inst_list.get('Oriel'):
		...
		elif self.inst_list.get('MS257_in') and self.inst_list.get('MS257_out') and self.inst_list.get('K2001A') and self.inst_list.get('Agilent34972A') and self.inst_list.get('Oriel'):
	...
	def update1(self,pyqt_object):
		...
		self.new_position_.extend([format(float(new_position)*1e9, '.2f')])
		...
		if self.write2db_check:
			...
			self.db.execute(''.join(["INSERT INTO spectra VALUES (",format(float(new_position)*1e9, '.2f'),",", all_data, ",'", pos,"',", str(shut_bin), ",", timelist, ",'", dateandtime, "')"]))
		...
		if self.write2txt_check:
			...
			with open(self.textfile, 'a') as thefile:
				thefile.write("%s " %format(float(new_position)*1e9, '07.2f'))
		...
		# add row elements
		self.tableWidget.setItem(self.tal, 0, QTableWidgetItem(format(float(new_position)*1e9, '.2f') ))
		self.tableWidget.setItem(self.tal, 1, QTableWidgetItem(format(float(all_data), '.3e' )) )
		self.tableWidget.setItem(self.tal, 2, QTableWidgetItem(pos) )
		self.tableWidget.setItem(self.tal, 3, QTableWidgetItem(shutter) )
		self.tableWidget.setItem(self.tal, 4, QTableWidgetItem(format(float(timelist),'.2f') ))
	...
	def update2(self,pyqt_object):
		...
		self.all_wl_tr.extend([ float(all_positions) ])
		...
		if len(self.all_wl_tr)>self.schroll:
			...
			self.plot_wl_tr[-1] = float(all_positions)
			...
			self.plot_wl_tr.extend([ float(all_positions) ])
		...
		self.curve5.setData(self.plot_time_tr, self.plot_volts_tr)
