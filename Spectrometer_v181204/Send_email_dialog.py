...
import os, re, imp, serial, time, yagmail, zipfile, configparser
...
class Send_email_dialog(QDialog):
	...
	def btn_send_email(self):
		...
		try:
			...
			self.yag.send(self.emailrec_str, "File(s) sent from the control computer", contents=self.all_files)
			...
			QMessageBox.critical(self, 'Message',''.join(["Could not send e-mail to the gmail account ",self.emailset_str[0],"! Try following steps:\n1. Check your internet connection. \n2. Check the account username and password.\n3. Make sure that the account accepts less secure apps.\n\n",str(e)]))
