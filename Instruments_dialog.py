...
from instruments import MS257, K2001A, A34972A, Oriel_stepper, GUV
...
class Instruments_dialog(QDialog):
	...
	def __init__(self, parent, inst_list, timer):
		...
		try:
			...
			self.guvip_str=self.config.get("Instruments",'guvport').strip().split(',')[0]
			self.guvport_str=self.config.get("Instruments",'guvport').strip().split(',')[1]
			self.guvport_check=self.bool_(self.config.get("Instruments",'guvport').strip().split(',')[2])
    ...
    ##############################################
		...
		if self.inst_list.get("GUV"):
			...
			self.guv_status.setText("Status: disconnected")
		...
		if not self.checked_list.get("A34972A"):
	...
	def set_connect(self):
		...
		if self.cb_ims257.isChecked():
			if not self.inst_list.get("MS257_in"):
				try:
					self.MS257_in = MS257.MS257(self.ms257inport_str, self.cb_testmode.isChecked())
					self.MS257_in.set_timeout(1)
					print(self.MS257_in.getVersion())
					self.MS257_in.set_timeout(60)
				except Exception as e:
					QMessageBox.critical(self, 'MS257 input', ''.join(["MS257 input monochromator could not return valid echo signal. Check the port name and check the connection.\n",str(e),"\n\nOtherwise, try and run in the TEST MODE!"]))
					...
					self.inst_list.update({"MS257_in_tm":self.cb_testmode.isChecked()})
			else:
				self.ims257_status.setText("Status: already CONNECTED")
		else:
			if self.inst_list.get("MS257_in"):
				...
				self.ims257_status.setText("Status: already disconnected!")
				
		###############################################################
		
		if self.cb_oms257.isChecked():
			if not self.inst_list.get("MS257_out"):
				try:
					self.MS257_out = MS257.MS257(self.ms257outport_str, self.cb_testmode.isChecked())
					self.MS257_out.set_timeout(1)
					print(self.MS257_out.getVersion())
					self.MS257_out.set_timeout(60)
				except Exception as e:
					QMessageBox.critical(self, 'MS257 output', ''.join(["MS257 output monochromator could not return valid echo signal. Check the port name and check the connection.\n",str(e),"\n\nOtherwise, try and run in the TEST MODE!"]))
					...
					self.inst_list.update({"MS257_out_tm":self.cb_testmode.isChecked()})
			else:
				self.oms257_status.setText("Status: already CONNECTED")
		else:
			if self.inst_list.get("MS257_out"):
				...
				self.oms257_status.setText("Status: already disconnected!")
		...
		if self.cb_oriel.isChecked():
			if not self.inst_list.get("Oriel"):
				try:
					self.Oriel = Oriel_stepper.Oriel_stepper(self.orielport_str, self.cb_testmode.isChecked())
					...
					QMessageBox.critical(self, 'Oriel stepper', ''.join(["Oriel stepper could not return valid echo signal. Check the port name and check the connection.\n",str(e),"\n\nOtherwise, try and run in the TEST MODE!"]))
					...
					self.inst_list.update({"Oriel_tm":self.cb_testmode.isChecked()})
			else:
				self.oriel_status.setText("Status: already CONNECTED")
		else:
			if self.inst_list.get("Oriel"):
				...
				self.oriel_status.setText("Status: already disconnected!")
				
		###############################################################
		
		if self.cb_k2001a.isChecked():
			if not self.inst_list.get("K2001A"):
				try:
					self.K2001A = K2001A.K2001A(self.k2001aport_str, self.cb_testmode.isChecked())
					print(self.K2001A.return_id())
				except Exception as e:
					QMessageBox.critical(self, 'Keithley 2001A', ''.join(["Keithley 2001A could not return valid echo signal. Check the port name and check the connection.\n",str(e),"\n\nOtherwise, try and run in the TEST MODE!"]))
					...
					self.inst_list.update({"K2001A_tm":self.cb_testmode.isChecked()})
			else:
				self.k2001a_status.setText("Status: already CONNECTED")
		else:
			if self.inst_list.get("K2001A"):
				...
				self.k2001a_status.setText("Status: already disconnected!")
				
		###############################################################
		
		if self.cb_a34972a.isChecked():
			if not self.inst_list.get("A34972A"):
				try:
					self.A34972A = A34972A.A34972A(self.a34972aport_str, self.cb_testmode.isChecked())
					print(self.A34972A.return_id())
				except Exception as e:
					QMessageBox.critical(self, 'Agilent 34972A', ''.join(["Agilent 34972A could not return valid echo signal. Check the port name and check the connection.\n",str(e),"\n\nOtherwise, try and run in the TEST MODE!"]))
					if hasattr(self,'A34972A'):
						self.A34972A.close()
				else:
					self.a34972a_status.setText("Status: CONNECTED")
					self.a34972a_status.setStyleSheet("color: green")
					self.inst_list.update({"A34972A":self.A34972A})
					self.inst_list.update({"A34972A_tm":self.cb_testmode.isChecked()})
			else:
				self.a34972a_status.setText("Status: already CONNECTED")
		else:
			if self.inst_list.get("A34972A"):
				self.inst_list.get("A34972A").close()
				self.inst_list.pop("A34972A", None)
				self.a34972a_status.setText("Status: disconnected!")
				self.a34972a_status.setStyleSheet("color: red")
			else:
				self.a34972a_status.setText("Status: already disconnected!")
				
		###############################################################
		
		if self.cb_guv.isChecked():
			if not self.inst_list.get("GUV"):
				try:
					self.GUV = GUV.GUV([self.guvip_str,self.guvport_str,self.guvtype_str], self.cb_testmode.isChecked(), None)
					print(self.GUV.return_id())
				except Exception as e:
					QMessageBox.critical(self, self.guvtype_str, ''.join([self.guvtype_str," could not return valid echo signal. Check the port name and check the connection.\n",str(e),"\n\nOtherwise, try and run in the TEST MODE!"]))
					if hasattr(self,'GUV'):
						self.GUV.close()
				else:
					self.guv_status.setText("Status: CONNECTED")
					...
					self.inst_list.update({"GUV_tm":self.cb_testmode.isChecked()})
			else:
				self.guv_status.setText("Status: already CONNECTED")
		else:
			if self.inst_list.get("GUV"):
				...
				self.guv_status.setText("Status: already disconnected!")
				
		###############################################################
			
		keys = self.inst_list.keys() & ["MS257_in","MS257_out","Oriel","K2001A","A34972A","GUV"]
		if not keys:
	...
	def save_(self):
		...
		self.config.read('config.ini')
		self.config.set("Instruments", 'ms257inport', ','.join([str(self.ims257Edit.text()), str(self.cb_ims257.isChecked()) ]) )
		self.config.set("Instruments", 'ms257outport', ','.join([str(self.oms257Edit.text()), str(self.cb_oms257.isChecked())]) )
		self.config.set("Instruments", 'orielport', ','.join([str(self.orielEdit.text()), str(self.cb_oriel.isChecked()) ]) )
		self.config.set("Instruments", 'k2001aport', ','.join([str(self.k2001aEdit.text()), str(self.cb_k2001a.isChecked())]) )
		self.config.set("Instruments", 'a34972aport', ','.join([str(self.a34972aEdit.text()), str(self.cb_a34972a.isChecked())]) )
		self.config.set("Instruments", 'testmode', str(self.cb_testmode.isChecked()) )
		self.config.set("Instruments", 'guvport', ','.join([str(self.guvipEdit.text()), str(self.guvportEdit.text()), str(self.cb_guv.isChecked())]) )
		self.config.set("Instruments", 'guvtype', self.guvtype_str)
