...
class GUV_TEST_dialog(QDialog):
	...
	def setupUi(self):
		...
		self.cb_logdata = QCheckBox('Log data',self)
		self.cb_logdata.toggle()
		self.cb_logdata.setChecked(self.log_guv)
		#self.cb_logdata.setLayoutDirection(Qt.RightToLeft)
		self.my_file = ''.join(['Log data to file: txt/All_GUVdata_',self.time_str,'.txt'])
		self.file_lbl = QLabel(self.my_file,self)
		...
		g0_3 = QHBoxLayout()
		g0_3.addWidget(self.cb_logdata)
		g0_3.addWidget(self.lcd)
		
		g0_3_ = QGridLayout()
		g0_3_.addLayout(g0_3,0,0)
		g0_3_.addWidget(self.file_lbl,1,0)
		
		g0_4 = QVBoxLayout()
		g0_4.addLayout(g0_1)
		g0_4.addLayout(g0_2)
		g0_4.addLayout(g0_3)
		g0_4.addLayout(g0_3_)
	...
	def logdata(self):
		...
		if self.cb_logdata.isChecked():
			self.config.set('DEFAULT','log_guv', str(True) )
			self.time_str=time.strftime("%y%m%d-%H%M")
			self.lcd.display(self.time_str)
			self.config.set('DEFAULT','timetrace', self.time_str )
			self.my_file = ''.join(['Log data to file: txt/All_GUVdata_',self.time_str,'.txt'])
			self.file_lbl.setText(self.my_file)
		else:
			self.config.set('DEFAULT','log_guv', str(False) )
