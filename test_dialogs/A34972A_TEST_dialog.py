...
class A34972A_Worker(QRunnable):
	...
	def run(self):
		...
		while not self.abort_flag:
			X_val=self.a34972a.return_voltage()
...
class A34972A_TEST_dialog(QDialog):
	...
	def __init__(self, parent):
		...
		try:
			...
			self.a34972aport=self.config.get("Instruments",'a34972aport').strip().split(',')[0]
	...
	def setupUi(self):
		...
		print("Multithreading in the Agilent 34972A dialog with maximum %d threads" % self.threadpool.maxThreadCount())
	...
	def set_run(self):
		...
		try:
			self.A34972A = A34972A.A34972A(str(self.serialportEdit.text()), False)
		except Exception as e:
			reply = QMessageBox.critical(self, 'Agilent 34972A TEST MODE', ''.join(["Agilent 34972A could not return valid echo signal. Check the port name and check the connection.\n",str(e),"\n\nShould the program proceeds into the TEST MODE?"]), QMessageBox.Yes | QMessageBox.No)
			if reply == QMessageBox.Yes:
				self.A34972A = A34972A.A34972A(str(self.serialportEdit.text()), True)
			else:
				return
		...
		setrun_obj=type('setscan_obj',(object,),{ 'a34972a':self.A34972A })
