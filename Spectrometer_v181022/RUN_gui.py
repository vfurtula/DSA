...
class TEST_Worker(QRunnable):
		...
		for i,j,k,l in zip(self.sssd[0],self.sssd[1],self.sssd[2],self.sssd[3]):
			...
			for new_position in wv_scanlist:
				...
				if self.inst_list.get('MS257_in'):
					...
					self.ms257_in=float(format(ms257_in, '.5e'))
				else:
					self.ms257_in=-1
					
				if self.inst_list.get('MS257_out'):
					ms257_out=self.inst_list.get('MS257_out').getCurrentWL()
					self.ms257_out=float(format(ms257_out, '.5e'))
				else:
					self.ms257_out=-1
				
				self.step_wl=new_position
				...
				if self.pos=="A<->B":
					...
					if self.shutset=="on->off":
						...
						self.my_legend = "A-<font>&Delta;</font>"
					...
					if self.shutset=="on->off":
						...
						self.my_legend = "B-<font>&Delta;</font>"
						self.update_end_vals = True
						self.read_signals(obj)
	...
	def read_signals(self,*tupl):
		...
		if self.inst_list.get('Oriel') and hasattr(self,'mirrorPosA'):
			if self.mirrorPosA:
				time_start_=time.time()
				while (time.time()-time_start_)<self.posA_delay and not self.abort_flag:
					time.sleep(0.01)
		elif self.inst_list.get('Oriel') and hasattr(self,'mirrorPosB'):
			if self.mirrorPosB:
				time_start_=time.time()
	...
	def close_shutter(self):
		...
		if hasattr(self,'light'):
			if self.inst_list.get('MS257_in') and self.light:
	...
	def open_shutter(self):
		...
		if hasattr(self,'dark'):
			if self.inst_list.get('MS257_in') and self.dark:
	...
	def set_posA(self):
		...
		if hasattr(self,'mirrorPosB'):
			if self.inst_list.get('Oriel') and self.mirrorPosB:
				self.inst_list.get('Oriel').goto_a()
				self.signals.update_oriel.emit("A")
				time_start_=time.time()
				while (time.time()-time_start_)<2.5 and not self.abort_flag:
					time.sleep(0.01)
				print("Mirror position A")
				self.mirrorPosA=True
				self.mirrorPosB=False
		else:
	...
	def set_posB(self):
		...
		if hasattr(self,'mirrorPosA'):
			if self.inst_list.get('Oriel') and self.mirrorPosA:
				...
				self.mirrorPosA=False
				self.mirrorPosB=True
		else:
			if self.inst_list.get('Oriel'):
				self.inst_list.get('Oriel').goto_b()
				self.signals.update_oriel.emit("B")
...
class Run_TEST(QMainWindow):
	...
	def initUI(self):
		...
		self.c_a34972a_all_Aoff = self.p1.plot(pen=pg.mkPen('r',width=2, style=QtCore.Qt.SolidLine), symbol='d', symbolPen='r', symbolBrush='r', symbolSize=3)
		...
		self.c_a34972a_all_Boff = self.p1.plot(pen=pg.mkPen('r',width=2, style=QtCore.Qt.SolidLine), symbol='o', symbolPen='r', symbolBrush='r', symbolSize=3)
	...
	def update_end_k2001a(self,pyqt_object):
		...
		if self.shutset in ["on->off","on<->off"]:
			self.shutter_.extend([self.shutset])
		else:
		...
		if self.shutset in ["on->off","on<->off"]:
			self.tableWidget.setItem(self.tal, 4, QTableWidgetItem(u'\u0394') )
		else:
